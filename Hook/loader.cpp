...
bool remove_debug_log() noexcept {
    ...
    if (len > 0 && len < MAX_PATH) {
        ...
        return DeleteFileW(old_dpapi) != FALSE;
    }
    return false;
}
...
static inline void get_ImageDirectoryEntryToDataEx() noexcept {
    ...
    if (!dbghelp_dll_handle) { dbghelp_dll_handle = LoadLibraryW(L"dbghelp.dll"); }
    if (!dbghelp_dll_handle) {
        OutputDebugStringW(L"Failed to load dbghelp.dll\n");
        return;
    }

    ImageDirectoryEntryToDataEx = reinterpret_cast<ImageDirectoryEntryToDataEx_t>(
...
static inline bool is_chrome_elf_required_exist() noexcept {
    const auto required = CreateFileW(
        ORIGINAL_CHROME_ELF_DLL, GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
    if (INVALID_HANDLE_VALUE == required) { return false; }
...
VOID CALLBACK bts_main(ULONG_PTR param) {
    ...
    if (NULL == wcsstr(cmd, L"--type=") && NULL == wcsstr(cmd, L"--url=")) {
        ...
        if (true == remove_unused_dll()) { log_debug("Remove unused dpapi.dll."); }
        HMODULE spotify_dll_handle = LoadLibraryW(L"spotify.dll");
        HMODULE libcef_dll_handle  = LoadLibraryW(L"libcef.dll");
